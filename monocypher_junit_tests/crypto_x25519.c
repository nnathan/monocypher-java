...
int main(int argc, char **argv) {
  {
    ...
    crypto_x25519(raw, your_secret, their_public);

    char raw_hex_buf[64 + 1] = {0};
    char secret_hex_buf[64 + 1] = {0};
    char public_hex_buf[64 + 1] = {0};
    to_hex_string(your_secret, 32, secret_hex_buf);
    to_hex_string(their_public, 32, public_hex_buf);
    to_hex_string(raw, 32, raw_hex_buf);

    printf("your secret: %s\n", secret_hex_buf);
    printf("their public: %s\n", public_hex_buf);
    printf("their raw: %s\n", raw_hex_buf);
  }
